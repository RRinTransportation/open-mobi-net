...
def load_network(project_save_path = 'data/Lyon/raw_data/net_pt'):
    pt_links = gpd.read_file(f"{project_save_path}/lines/lines.shp")
    stops = gpd.read_file(f"{project_save_path}/stops/stops.shp")
    unique_routes = pt_links.drop_duplicates(subset=['route_id'])
    return pt_links,stops,unique_routes

def restrain_to_area_study(pt_links,pt_nodes,study_area_polygon):

    gdf_right = gpd.GeoDataFrame(geometry=[study_area_polygon.exterior], crs=pt_links.crs)

    pt_links_intersecting = gpd.sjoin(pt_links, gdf_right, how='inner', predicate='intersects')
    pt_nodes_inside = gpd.sjoin(pt_nodes, gdf_right, how='inner', predicate='intersects')

    # --- Keep only part of routes which are inside the polygon:
    pt_links_inside = pt_links_intersecting.copy()
    ...
    pt_links_inside = pt_links_inside[pt_links_inside.geometry.type.isin(['LineString', 'MultiLineString'])]
    # ---

    return pt_links_inside, pt_links_intersecting, pt_nodes_inside

def get_candidates_links(road_links,bus_routes_buffered):
    """
...
def spatial_matching_bus_to_road(candidate_matches, bus_routes, road_links):
    """
    Matches bus routes to the best path of road links using graph theory.
    Steps:
    1. Identify potential extremity links in the candidate set.
    2. Determine which of these links are near the true start/end of each bus route.
    3. Filter out candidate links that are graph extremities but not true bus route extremities.
    4. For each bus route, find the best path among the filtered candidates using Dijkstra's algorithm.
    """

    # 1. Find all potential extremities in the entire candidate set
    possible_extremities = find_possible_extremities(candidate_matches)

    # 2. Identify which links are near the true start/end of each bus route
    bus_extremities_dict = find_bus_route_extremities(candidate_matches, bus_routes,10)

    # 3. Create a set of all links that are confirmed bus route extremities
    confirmed_extremity_links = set()
    for route_id in bus_extremities_dict:
        confirmed_extremity_links.update(bus_extremities_dict[route_id]['start_bus_lane'])
        confirmed_extremity_links.update(bus_extremities_dict[route_id]['end_bus_lane'])

    # 4. Filter out candidate links that are graph extremities but not true bus route extremities
    bad_extremities = possible_extremities - confirmed_extremity_links
    # filtered_candidates = candidate_matches[~candidate_matches['link_id'].isin(list(bad_extremities))]
    filtered_candidates = candidate_matches.copy()

    # 5. For each route, find the best path among the filtered candidates
    final_link_indices = []
    ...
    for sub_route_id, route_extremities in tqdm(bus_extremities_dict.items(), desc="Processing Bus Routes"):
        route_candidates = filtered_candidates[filtered_candidates[sub_route_idx_col] == sub_route_id].copy()
        ...
        assert len(route_ids) == 1, "Multiple line_name found for a single bus route segment"
        ...
        best_path = find_best_path_for_route_optimized(route_candidates, start_links, end_links, bus_geom)
        if len(best_path) > 2:
    ...
    final_pt_links = final_pt_links.to_crs(epsg=4326)
    final_matches = final_matches.to_crs(epsg=4326)
    return final_matches, final_pt_links


def find_possible_extremities(candidates_gdf,link_id_col = 'link_id'):
    """
...
def find_bus_route_extremities(candidates_gdf, bus_routes_gdf, epsilon=3):
    """
...
def find_best_path_for_route(route_candidates_gdf, start_link_ids, end_link_ids, bus_route_geom):
    """
...
def calculate_edge_weight(road_geom, bus_geom):
    """
...
def find_best_path_for_route_optimized(route_candidates_gdf, start_link_ids, end_link_ids, bus_route_geom):
    """
    ...
    route_candidates_gdf['weight'] = route_candidates_gdf['geometry'].apply(
        lambda geom: calculate_edge_weight(geom, bus_route_geom)
    )
